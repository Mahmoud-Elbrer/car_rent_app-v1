...
class DashboardApiServices {
  ...
  static Future<ProfileInfoModel?> dashboardApiProcess() async {
    ...
    try {
      mapResponse = await ApiMethod(isBasic: false).get(ApiEndpoint.dashboardURL, code: 200, showResult: true);
      if (mapResponse != null) {
  ...
  static Future<CarListModel?> carListProcess() async {
    ...
    try {
      mapResponse =
          await ApiMethod(isBasic: !LocalStorage.isLoggedIn()).get(ApiEndpoint.carListURL, code: 200, showResult: true);
      if (mapResponse != null) {
  ...
  static Future<CarBookingModel?> carBookingProcessApi({required Map<String, dynamic> body}) async {
    Map<String, dynamic>? mapResponse;
    try {
  ...
  static Future<CarBookingModel?> carBookingConfirmApi({required String token}) async {
    Map<String, dynamic>? mapResponse;
    try {
  ...
  static Future<BookingVerifyModel?> bookingVerificationProcessApi({required Map<String, dynamic> body}) async {
    Map<String, dynamic>? mapResponse;
    try {
      mapResponse = await ApiMethod(isBasic: !LocalStorage.isLoggedIn()).post(
  ...
  ///* Forgot Password Process Api
  ///todo check
  static Future<BookingMailOtpResendModel?> resentOtpProcess({required String token}) async {
    Map<String, dynamic>? mapResponse;
    try {
      mapResponse = await ApiMethod(isBasic: !LocalStorage.isLoggedIn()).get(
      ...
      if (mapResponse != null) {
        BookingMailOtpResendModel result = BookingMailOtpResendModel.fromJson(mapResponse);
        CustomSnackBar.success(
          result.message.success.success.first.toString(),
        );
        return result;
  ...
  ///* car area type url
  static Future<CarAreaTypeModel?> carAreaTypesApi({required Map<String, dynamic> body}) async {
    Map<String, dynamic>? mapResponse;
    try {
      mapResponse = await ApiMethod(isBasic: !LocalStorage.isLoggedIn()).post(
  ...
  ///* car area type url
  static Future<SearchCarModel?> carSearchApi({required Map<String, dynamic> body}) async {
    Map<String, dynamic>? mapResponse;
    try {
      mapResponse = await ApiMethod(isBasic: !LocalStorage.isLoggedIn()).post(
