...
import '../../model/profile/delete_profile_model.dart';
...
class ProfileApiServices {
  ...
  ///* update profile process api
  static Future<DeleteAccountModel?> deleteProfileAPi(
      {required Map<String, dynamic> body}) async {
    Map<String, dynamic>? mapResponse;
    try {
      mapResponse = await ApiMethod(isBasic: false)
          .post(ApiEndpoint.profileDeleteURL, body, code: 200);
      if (mapResponse != null) {
        DeleteAccountModel result = DeleteAccountModel.fromJson(mapResponse);
        CustomSnackBar.success(result.message.success.first.toString());
        return result;
      }
    } catch (e) {
